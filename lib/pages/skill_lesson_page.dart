...
import 'package:newskilloapp/pages/communication_practice_page.dart';
...
class _SkillLessonPageState extends State<SkillLessonPage> {
  ...
  void _nextLesson() {
    if (_currentLessonIndex < _lessons.length - 1) {
      ...
      // Branch navigation based on skill type
      final title = selectedSkill?['title']?.toString().toLowerCase() ?? '';
      final isCommunication = title.contains('communication') || 
      ...
      if (isCommunication) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => CommunicationPracticePage(
              skillTitle: selectedSkill?['title'] ?? 'Communication',
            ),
          ),
        );
      } else {
